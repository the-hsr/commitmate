...
var APIClient = class {
  constructor({
    ...
    __classPrivateFieldSet5(this, _APIClient_baseURLOverridden, baseURLOverridden, "f");
  ...
  buildURL(path, query, defaultBaseURL) {
    const baseURL = !__classPrivateFieldGet6(this, _APIClient_baseURLOverridden, "f") && defaultBaseURL || this.baseURL;
...
function activate(context) {
  let disposable = vscode.commands.registerCommand("gitCommitAI.generateCommitMessage", async () => {
    try {
      ...
      if (!commitType) {
        vscode.window.showWarningMessage("Commit type not selected.");
        return;
      }
      const prompt = `
      ...
      let commitMessage = completion.choices[0]?.message?.content?.trim() || "";
      if (!commitMessage) {
...
${commitMessage}`;
      ...
      outputChannel.show();
      vscode.window.showInformationMessage(`Commit message generated as ${formattedTitle} and copied to clipboard (see 'AI Commit Message' panel).`);
    } catch (err) {
...
async function getApiKey(context) {
  let apiKey = context.globalState.get("groqApiKey");
  if (!apiKey) {
...
async function getGitDiff() {
  return new Promise((resolve, reject) => {
    (0, import_child_process.exec)("git diff --staged", { cwd: vscode.workspace.rootPath }, (error, stdout) => {
